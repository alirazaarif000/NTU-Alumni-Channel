...
export const deleteConversation = ({ conversationId, userId, message, auth }) => async (dispatch) => {
  try {
    const res = await getDataAPI(`deleteconversations/${conversationId}`, auth.token);
    const user = message.users.find(user => user._id === userId);
    dispatch({ type: MESSAGE_TYPES.DELETE_CONVERSATIONS, payload: { ...user, text: '', media: [] } });
