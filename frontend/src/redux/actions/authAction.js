...
export const changePassword = ({ oldPassword, newPassword, cnfNewPassword, auth }) => async (dispatch) => {
  ...
  if (newPassword !== cnfNewPassword) {
    return dispatch({
      type: GLOBALTYPES.ALERT,
      payload: { error: "Your confirm password does not matched" },
    });
  }
...
export const updateProfileAdmin = ({ formData, auth }) => async (dispatch) => {
  ...
  try {
    const res = await patchDataAPI("user", { ...formData }, auth.token);
    dispatch({
      type: GLOBALTYPES.AUTH,
      payload: {
        ...auth,
