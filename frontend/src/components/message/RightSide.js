...
import { addMessage, getMessages, MESSAGE_TYPES, deleteConversation, getConversations } from '../../redux/actions/messageAction';
import LoadIcon from '../../images/loading.gif';
import { useHistory } from 'react-router-dom';

const RightSide = ({ setShowChats }) => {
  ...
  return (
    <div className='border h-100'>
      <div className="message_header pe-2 d-flex align-items-center">
        <div
          className="backbutton2 text-black fs-5 cursor-pointer m-0 ms-1"
        ><i class="fa fa-arrow-left" aria-hidden="true" onClick={() => { setShowChats(true) }}></i>
        </div>
        {user.length !== 0 && (
          <UserCard user={user}>
            <i className={`fas fa-trash text-danger cursor-pointer ${conversationId ? "" : "d-none"}`} onClick={handleDelete} />
          </UserCard>
        )}
      </div>
            ...
            <div key={index}>
              ...
              {msg.sender === auth.user._id && (
                <div className="chat_row you_message">
                  <MsgDisplay user={{ ...auth.user, username: "You" }} msg={msg} theme={theme} />
                </div>
              )}
            </div>
          ))}
