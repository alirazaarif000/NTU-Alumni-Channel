...
const UserCard = ({
  ...
  return (
    <div
      className={`d-flex justify-content-between pe-2 px-md-2 py-1 w-100 align-items-center ${border}`}
    >
      <div>
        <Link
          to={`/profile/${user._id}`}
        ...
        >
          ...
          <div className="ms-2" style={{ transform: "translateY(-2px)" }}>
            <span className="d-block color-c2">{truncatedUsername(user.username)}</span>
